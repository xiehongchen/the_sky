...
Mock.mock('/mock/userinfo', 'post', (params) => {
  ...
  const findUserToken = user.userToken.find(
    (item: USERTOKEN) => item.token == token,
  )
  if (findUserToken) {
    const findUserInfo = user.userInfo.find(
      (item: USERINFO) => item.userId == findUserToken.userId,
    )
