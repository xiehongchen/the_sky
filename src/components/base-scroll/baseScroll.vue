<template>
  <div class="scroll-box" ref="scrollBox">
    <div
      ...
      ref="scroll"
      :style="{
        height: scrollModuleHeight + 'px',
        top: scrollModuleTop + 'px',
      }"
...
// 滚动模块高度
const scrollModuleHeight = computed<number>(
  () => (props.showHeight / props.scrollHeight) * scrollBoxHeight.value,
)
// 滚动模块的位置
const scrollModuleTop = computed<number>(
  () => (props.scrollTop / props.scrollHeight) * scrollBoxHeight.value,
)
  ...
  nextTick(() => {
    ...
    scroll.value.addEventListener('mousedown', (event: MouseEvent) => {
      ...
      document.onmousemove = (e: MouseEvent) => {
        ...
        // 计算滚动后距离顶部距离
        let top =
          ((clientY2 - clientY) / scrollBoxHeight.value) * props.scrollHeight +
          clickTop
        // 判断滚动方向
        if (top > 0) {
          // 判断是否超过界限
